...
#	Required Files
$gene_ontology_file = '../data/static/POT.obo';
...
if ((not(-s $gene_ontology_file)) || (not(-T $gene_ontology_file))) {
	die "\n\tRequired Pfizer Tissue Ontology file not found or in wrong format at '$gene_ontology_file'!\n\n";
}
else {
	...
	if (($obo_ver eq "") || ($obo_date eq "") || ($obo_date eq "")) {
		die "\n\tRequired Pfizer Tissue Ontology file in wrong format at '$gene_ontology_file'!\n\n";
	}
}
$go_file_desc = "Pfizer Ontology of Tissues built using file '$obo_file_list'."
					. " Files headings indicated that the format version was: '$obo_ver',"
					. " the datestamp was: '$obo_date',"
					. " and the creator was: '$obo_creator'.";

$dsn = $MiscVariables{DATABASE};
...
my $class_sys_name = make_sql_safe('Pfizer Ontology of Tissues');
...
#				$PHENO{$current_id}{ALTS} .= "\|$alt_id";
	...
	if ($line =~ /^relationship: part_of (POT\d+)/) {
		my $parent_id = $1;
		$parent_id =~ s/^POT0+//;
		$ParentChild{$parent_id}{$current_id} = 'part_of';
		$ChildParent{$current_id}{$parent_id} = 'part_of';
	}
	if ($line =~ /^relationship: derived_from (POT\d+)/) {
		my $parent_id = $1;
		$parent_id =~ s/^POT0+//;
		$ParentChild{$parent_id}{$current_id} = 'derived_from';
		$ChildParent{$current_id}{$parent_id} = 'derived_from';
	}
	if ($line =~ /^relationship: related_to (POT\d+)/) {
		my $parent_id = $1;
		$parent_id =~ s/^POT0+//;
		$ParentChild{$parent_id}{$current_id} = 'related_to';
		$ChildParent{$current_id}{$parent_id} = 'related_to';
	}
}
...
$cmd = 'mysql -P' . $port . ' -h' . $host . ' -u' . $username . ' -p' . $password . ' < ' . $sql_file;
`$cmd`;
