...
@Data_Files = qw( ./tmp_files/Human_Toxicity.PercentLogMax ./tmp_files/Mouse_Toxicity.PercentLogMax );
$percent_sql_file = './tmp_files/InsertPercentiles.sql';
open(PERCENT_OUTFILE, ">$percent_sql_file");
print PERCENT_OUTFILE "use $db_name;\n";
print PERCENT_OUTFILE "DELETE FROM Annotation WHERE Annot_Link = 'TOX_AT_GLANCE';\n";
foreach $filename (@Data_Files) {
	open(FILE, $filename);
	$line = <FILE>;	#	skip header
	while ($line = <FILE>) {
		chomp($line);
		my ($idgene, $entrez, $homolog, $genebook, $percentiles) = split(/\t/, $line);
		$percentiles = make_sql_safe($percentiles);
		print PERCENT_OUTFILE "INSERT INTO Annotation(idGene, Annot_Text, Annot_Link, Create_Time) VALUES ('$idgene', '$percentiles', 'TOX_AT_GLANCE', NOW());\n";
	}
	close(FILE);
}
