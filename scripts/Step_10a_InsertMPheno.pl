...
#  Company     Pfizer Inc.
#
#  Creation Date - March 19, 2009
#  Modified -
#
#  Function - load Mouse Mutant Phenotype classes into ToxGene
#					part 'a' loads the phenotype ontologies
#
################################################################
...
#	Required Files
$mouse_pheno_ontol_file = $SetupFiles{MOUPHENOONT};

#	check that the ontology file looks OK and gather basic info about the file
$obo_ver = $obo_date = $obo_creator = $obo_file_list = "";
if ((not(-s $mouse_pheno_ontol_file)) || (not(-T $mouse_pheno_ontol_file))) {
	die "\n\tRequired Gene Ontology file not found or in wrong format at '$mouse_pheno_ontol_file'!\n\n";
}
else {
	$obo_file_list = `ls -l $mouse_pheno_ontol_file`;
	$obo_file_list =~ s/[\n\r\f]+//g;
	open (FILE, $mouse_pheno_ontol_file);
	...
	if (($obo_ver eq "") || ($obo_date eq "") || ($obo_date eq "")) {
		die "\n\tRequired Gene Ontology file in wrong format at '$mouse_pheno_ontol_file'!\n\n";
	}
}
...
####	load the mammalian phenotype ontologies
my %ParentChild = ();
my %ChildParent = ();
my %PHENO = ();
open (FILE, $mouse_pheno_ontol_file);
while ($line = <FILE>) {
	...
	if ($line =~ /^alt_id:\s+(MP:\d+)/) {
#				$PHENO{$current_id}{ALTS} .= "\|$1";
	}

	if ($line =~ /^is_a: (MP:\d+)/) {
		$ParentChild{$1}{$current_id} = 'is_a';
		$ChildParent{$current_id}{$1} = 'is_a';
	}
...
$sql_file = './tmp_files/MousePheno_a_1.sql';
...
$sql_file = './tmp_files/MousePheno_a_2.sql';
open(OUTFILE, ">$sql_file");
print OUTFILE "use $db_name;\n";
print OUTFILE "SET autocommit=0;\n";
print OUTFILE "START TRANSACTION;\n";
